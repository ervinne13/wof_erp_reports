			...
			<form id="<?=$this->uri->segment(3)?>-form" class="form-horizontal row page-form" role="form" class="container-fluid">
				<span class="col-md-6">
					<div class="form-group">
					  <label for="sel1" class="control-label col-xs-5">Purpose ID:</label>
					  <div class="col-xs-7">
				        <input type="text" class="form-control" id="" maxlength="30" value="<?= $data['R_Id']; ?>" tabindex="1" name="R_Id" placeholder="Reason ID">
				      </div>
					</div>
				    <div class="form-group">
				      <label class="control-label col-xs-5" for="">Description:</label>
				      <div class="col-xs-7">
				        <input type="text" class="form-control" id="" maxlength="100" value="<?= TRIM($data['R_Description']); ?>" tabindex="2" name="R_Description" placeholder="Description">
				      </div>
				    </div>
					<div class="form-group">
				      	<label class="control-label col-xs-5" for="">Modules:</label>
				      	<div class="col-xs-7">
					      	<select multiple class="form-control single-default select-mul" id="" name="RM_FK_Module_id[]" tabindex="3">
							  	<option value="" disabled selected>Modules</option>
							  	<?php 
							  		if(!empty($module['data'])){
							  			foreach ($module['data'] as $key => $value) {
							  	?>
							  		<option value="<?=$value['M_Module_id']?>" <?= in_array($value['M_Module_id'], array_column($data['module']['data'],'RM_FK_Module_id') )?"selected":"" ?> ><?=$value['M_Description']?></option>
							  	<?php }} ?>
							</select>
						</div>
				    </div>
