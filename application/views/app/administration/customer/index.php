...
var	table = $('#tbl-'+_class).bind('dynatable:init', function(e, dynatable) {
    	...
    	$('.activate').on('click',function(){
			...
			confirm("Activate Account?", function(confirmed) {
		   		if(confirmed){
					$.post(base_url + "app/administration/customer/process",data,function(){
						dynatable.process();
					});
				}
			});
		...
		$(document).on('click','.l-del',function(){
		 	_this = $(this);
		      confirm("Delete Record?", function(confirmed) {
		        if(confirmed){ 
		          $.post(base_url+'app/administration/customer/process',{id:_this.data('id'),type:'delete'},function(data){
		            if(data == 1){
		            	alert('Deleted!');
		            	setTimeout(function(){
						  dynatable.process();
		...
		dataset: {
			ajax: true,
		    ajaxUrl: base_url + "app/administration/customer/data",
		    ajaxOnLoad: true,
		    records: []
		},
		features: {
