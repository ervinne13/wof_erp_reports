			...
			<form id="<?=$this->uri->segment(3)?>-form" class="form-horizontal row page-form" role="form" class="container-fluid">
				<span class="col-md-6">
					<div class="form-group">
					  <label for="sel1" class="control-label col-xs-5">Supplier Posting Group:</label>
					  <div class="col-xs-7">
				        <input type="text" class="form-control" id="" maxlength="30" value="<?=$data['SPG_Code']?>" tabindex="1" name="SPG_Code" placeholder="Supplier Posting Group">
				    </div>
					</div>
				    <div class="form-group">
				      <label class="control-label col-xs-5" for="">Payable Account:</label>
				      <div class="col-xs-7">
					      <select class="form-control single-default select-cli" placeholder="Payable Account" id="" maxlength="30" name="SPG_COA_FK_AccountNo" tabindex="2">
							     <option value="" disabled selected>Payable Account</option>
							     <?php 
							  	    if(!empty($COA)){
							  		   foreach ($COA['data'] as $key => $value) {
							     ?>
							     <option value="<?=$value['COA_Account_id']?>" <?= trim($data['SPG_COA_FK_AccountNo'])==trim($value['COA_Account_id'])?'selected':'' ?> ><?=$value['COA_AccountName']?></option>
							     <?php }} ?>
							  </select>
						  </div>
				    </div>
