...
var	table = $('#tbl-customer-ledger').bind('dynatable:init', function(e, dynatable) {
    	$('#dynatable-search-tbl-customer-ledger').prepend('Date Filter: <input type="search" data-dynatable-query="date-from" name="date-from" id="date-from"/><input type="search" data-dynatable-query="date-to" name="date-to" id="date-to"/> ')
    	...
    	var options = $('#tbl-customer-ledger th').map(function () {
                return '<option value='+$(this).data('dynatable-column')+'>'+$(this).text()+'</option>';
                
            }).get();

    	$('#dynatable-per-page-tbl-customer-ledger').after(' Filter By: <select id="customer-filter"><option value="">-Select-</option>'+options+'</select>')
   		
   		$('#customer-filter').change(function(){
   		var value = $(this).val();
		  if (value === "") {
		    dynatable.queries.remove("customer-filer");
		  } else {
		    dynatable.queries.add("customer-filer",value);
		  }
		  dynatable.process();
   		});
		...
		dataset: {
			ajax: true,
		    ajaxUrl: base_url + "app/administration/customer/customer_ledger_data?id=<?= $this->input->get('id') ?>",
		    ajaxOnLoad: true,
		    records: []
		},
		features: {
