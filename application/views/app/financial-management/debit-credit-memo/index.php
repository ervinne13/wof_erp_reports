...
<script type="text/javascript">
    ...
    var table = $('.dynatable').bind('dynatable:init', function (e, dynatable) {
        $('#dynatable-search-tbl-debit-credit-memo').prepend('Date Filter: <input type="search" data-dynatable-query="date-from" name="date-from" id="date-from"/><input type="search" data-dynatable-query="date-to" name="date-to" id="date-to"/> ')
        ...
        dataset: {
            ajax: true,
            ajaxUrl: base_url + "app/" + moduleHeader +"/debit-credit-memo/data",
            ajaxOnLoad: true,
            records: []
        },
        features: {
        ...
        inputs: {processingText: '<img  id="loader" src="' + base_url + 'css/assets/data_loader.gif" />'
        }
    }).data('dynatable');
