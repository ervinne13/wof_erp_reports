...
var	table = $('#tbl-bank-ledger').bind('dynatable:init', function(e, dynatable) {
    	$('#dynatable-search-tbl-bank-ledger').prepend('Date Filter: <input type="search" data-dynatable-query="date-from" name="date-from" id="date-from"/><input type="search" data-dynatable-query="date-to" name="date-to" id="date-to"/> ')
    	.append("<a  href='javascript:void(0)' class='clear'>Clear</a>");

    	// var options = []; 
    	// $('#tbl-bank-ledger th').each(function(){
    	// 	options.push({id:$(this).data('dynatable-column'),value:$(this).text())})
    	// });

    	var options = $('#tbl-bank-ledger th').map(function () {
                return '<option value='+$(this).data('dynatable-column')+'>'+$(this).text()+'</option>';
                
            }).get();

    	$('#dynatable-per-page-tbl-bank-ledger').after(' Filter By: <select id="bank-filter"><option value="">-Select-</option>'+options+'</select>')
   		
   		$('#bank-filter').change(function(){
   		var value = $(this).val();
		  if (value === "") {
		    dynatable.queries.remove("bank-filer");
		  } else {
		    dynatable.queries.add("bank-filer",value);
		  }
		  dynatable.process();
   		});
		...
		dataset: {
			ajax: true,
		    ajaxUrl: base_url + "app/financial-management/bank-accounts/bank_ledger_data?id=<?= $this->input->get('id') ?>",
		    ajaxOnLoad: true,
		    records: []
		},
		features: {
