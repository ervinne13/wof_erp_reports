...
var	table = $('#tbl-item-re-class-journal').bind('dynatable:init', function(e, dynatable) {
    	$('#dynatable-search-tbl-item-re-class-journal').prepend('Date Filter: <input type="search" data-dynatable-query="date-from" name="date-from" id="date-from"/><input type="search" data-dynatable-query="date-to" name="date-to" id="date-to"/> ')
    	...
    	$('.activate').on('click',function(){
			...
			confirm("Activate Account?", function(confirmed) {
		   		if(confirmed){
					$.post(base_url + "app/financial-management/requisition/process",data,function(){
						dynatable.process();
					});
				}
			});
		...
		dataset: {
			ajax: true,
		    ajaxUrl: base_url + "app/"+moduleHeader+"/item-re-class-journal/data",
		    ajaxOnLoad: true,
		    records: []
		},
		features: {
